...
public class Fingerprinting {
	...
	public List<PositionTrace> getOfflineRadioMapMethodBased(){
		...
		for(TraceEntry entry: offlineTrace) {
			...
			if(!entrySet) {
				data.add(addSignalsMB(new PositionTrace(entry.getGeoPosition(),signals)));
			}
		}
		return data;
	}
	...
	public PositionTrace addSignalsMB(PositionTrace trace) {
		double p_d=0, d=0;
		for(int i=0; i<apPos.length;i++) {
			d = apPos[i].distance(trace.getPosition());
			p_d = -33.77-34.14*Math.log10(d/1);
